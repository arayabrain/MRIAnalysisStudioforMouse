...
def vbm_segment2(
    ...
    assert analysis_info_in.node_analysis_type == NodeAnalysisType.MASKING
    ...
    masking_dir_path = utils.get_derivatives_dir_path(project_path, params['analysis_id'], NodeAnalysisType.MASKING)

    # Create the output derivatives directory if it does not exist.
    segment2_dir_path = utils.create_derivatives_directory(project_path, params['analysis_id'],
                                                           NodeAnalysisType.SEGMENT2)
    ...
    unused_output_dir_path_list = utils.get_subdir_path_list(segment2_dir_path)

    # Perform the brain MRI image segmentation processing by Nipype-SPM12.
    analysis_info_out = process_vbm_segment2(analysis_info_in, masking_dir_path, segment2_dir_path,
                                             params['skip_analyzed'], vbm_config['segment2']['tpm_path'],
    ...
    create_dataset_description_file(function_name, masking_dir_path, segment2_dir_path, params['skip_analyzed'],
                                    vbm_config['segment2']['tpm_path'])
...
def process_vbm_segment2(analysis_info_in: AnalysisInfo, masking_dir_path: str, segment2_dir_path: str,
    ...
    Parameters
        ...
        masking_dir_path : str
            Path of the directory storing the analysis results performed by the vbm_masking node.
    ...
    analysis_info_out = analysis_info_in.create_new_analysis_info(NodeAnalysisType.SEGMENT2)
    ...
    for wf_input_path in analysis_info_in.workflow_input_file_path_list:
        ...
        try:
            ...
            if utils.skip_node_analysis(wf_input_path, skip_analyzed, previous_node_analysis_info):
                utils.remove_from_unused_list(join_filepath([segment2_dir_path, wf_input_name]),
            ...
            # Check a previous error.
                ...
                result = utils.check_derivatives_existence(masking_dir_path, wf_input_name)
                if not result['exist']:
                ...
                if utils.check_derivatives_existence(segment2_dir_path, wf_input_name)['exist']:
                    utils.delete_derivatives_data(segment2_dir_path, wf_input_name)

            # Segment brain MRI images.
            analysis_info_out.set_analysis_start_time(wf_input_path)
            output_dir_path = utils.create_directory([segment2_dir_path, wf_input_name])
            output_file_path_list = segment_brain_images(wf_input_name, masking_dir_path, segment2_dir_path, tpm_path)
...
def segment_brain_images(wf_input_name: str, masking_dir_path: str, segment2_dir_path: str, tpm_path: str) -> List[str]:
    ...
    templates = {'masking': '{subject_id}/{subject_id}.nii'}
    select_files_node = Node(SelectFiles(templates), name='select_files')
    select_files_node.inputs.base_directory = masking_dir_path
    ...
    sink_node.inputs.base_directory = segment2_dir_path

    # Create a workflow.
    wf = Workflow(name='vbm_segment2')
    wf.connect(select_files_node, 'masking', segment_node, 'channel_files')
    ...
    return utils.get_derivatives_file_paths(segment2_dir_path, wf_input_name)
...
def create_dataset_description_file(function_name: str, input_dir_path: str, output_dir_path: str,
    ...
    dataset_description = utils.get_dataset_description_template('2nd segmentation', function_name,
                                                                 input_dir_name_list)
